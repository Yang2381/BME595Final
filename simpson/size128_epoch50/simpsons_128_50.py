...
dataroot = "/home/nsakhala/bme595/simpson_dcgan/simpsons_dataset"
...
plt.imsave('init_images.png',np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=2, normalize=True).cpu(),(1,2,0)))
...
plt.imsave('real.png',np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True).cpu(),(1,2,0)))
...
#plt.show()
plt.imsave('fake.png',np.transpose(img_list[-1],(1,2,0)))
