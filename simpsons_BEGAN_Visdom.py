...
for epoch in range(num_epochs):
        for i, data in enumerate(dataloader, 0):
                ...
                #print(M.size(), done.size(), type(M), type(done))
                '''
                vis.line(X= torch.Tensor([done]), Y= M.detach().numpy(), opts=dict(
                	title = 'Convergence',
                	fillarea= True,
                	xlabel = 'iteration',
                ...
                if done % 1000 == 0:
                        ...
                        if win == None:
                        	win = vis.line(Y= measure_history, opts=dict(
			                	title = 'Convergence',
			                	fillarea= True,
			                	xlabel = 'iteration',
			                	ylabel = 'Measure of Convergence'))
                        else:
                        	win = vis.line(Y = measure_history, 
	                    		win = win)
                        save_checkpoint(i)
                        #save_image(gen_imgs,'BEGAN_%d.png'% done, nrow=8, normalize=True)
        if epoch%20 == 0:
                print("saving images")
